...
description: Platform-specific Compatibility of cargo buckal
---
...
1. Generate BUCK files:
   ...
   For first-time setup (initializes Buck2 at `<repo_root>` and generates BUCK files):

   ```bash
   cargo buckal migrate --init [<repo_root>]
   ```

   Or, for incremental updates (when Buck2 is already initialized):

   ```bash
   cargo buckal migrate
   ```

   This regenerates BUCK files based on current `Cargo.toml`/`Cargo.lock` changes without reinitializing Buck2. Use this after adding/removing dependencies or updating `Cargo.toml`.
...
2. Build with a target platform:
   ...
   Buckal configures a `buckal` cell ([Buckal bundles](https://github.com/buck2hub/buckal-bundles)). The bundles provide sample platforms under `//platforms:*`. You can also use your own platform definitions; any platform you use must include the appropriate OS constraint value (`prelude//os/constraints:windows` in the example above) so `select()` picks up the right `os_deps` branch.

   To update the pinned Buckal bundles revision, run:
